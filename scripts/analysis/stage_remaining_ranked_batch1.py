#!/usr/bin/env python3
"""Stage the first ranked batch of four unresolved episodes without production writes."""
...
DISCOVERY = REPO_ROOT / "data/audits/indicator_remaining_38_ranked_batch1.json"
DEFAULT_STAGE_DIR = REPO_ROOT / "work/remaining-ranked-batch1-staging"
REPORT = REPO_ROOT / "data/audits/indicator_remaining_ranked_batch1_staging_report.json"
...
def build_candidate(item: dict) -> dict:
    ...
    return {
        ...
        "original_reference_date": item["reference_date"],
        ...
        "catalog_correction": item.get("catalog_correction"),
        "provenance": [
            "indicator_remaining_38_ranked_batch1.json",
            "live NPR story page",
            "public-radio affiliate mirror",
...
def history_record(item: dict) -> dict:
    ...
    return {
        ...
        "description_provenance": "reference_title_fallback",
        "date_precision": "date_only",
        "recovery_status": "validated_remaining_ranked_batch1",
        "release_classification": item["release_classification"],
        "recovery_provenance": item["provenance"],
        "catalog_correction": item["catalog_correction"],
    }
...
def enclosure_record(item: dict) -> dict:
    ...
    return {
        ...
        "duration_seconds": item["duration_seconds"],
        "extraction_method": "validated_remaining_ranked_batch1",
...
def stage(repo_root: Path = REPO_ROOT, stage_dir: Path | None = None) -> dict:
    ...
    candidates = [build_candidate(item) for item in discovery["episodes"]]
    ids = {item["npr_story_id"] for item in candidates}
    if len(candidates) != 4 or len(ids) != 4:
        raise RuntimeError("Discovery must contain exactly four unique approved episodes.")
    ...
    corrected = next(item for item in candidates if item["npr_story_id"] == "703928648")
    checks = {
        "production_files_unchanged": before == after,
        "candidate_count_is_4": len(candidates) == 4,
        "candidate_story_ids_unique": len(ids) == 4,
        "history_delta_is_4": len(history["episodes"]) - history_before == 4,
        "history_count_consistent": history["episode_count"] == len(history["episodes"]),
        "enclosure_map_delta_is_4": len(enclosure["episodes"]) - map_before == 4,
        "feed_delta_is_4": len(parsed_items) - feed_before == 4,
        ...
        "catalog_date_correction_applied": corrected["reference_date"] == "2019-03-15",
    }
    report = {
        "report_version": 1,
        "mode": "staging_only",
