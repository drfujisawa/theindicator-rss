#!/usr/bin/env python3
"""Atomically apply the reviewed four-episode ranked staging set."""
...
DEFAULT_STAGE_DIR = REPO_ROOT / "work/remaining-ranked-batch1-staging"
STAGING_REPORT = REPO_ROOT / "data/audits/indicator_remaining_ranked_batch1_staging_report.json"
APPLICATION_REPORT = REPO_ROOT / "data/audits/indicator_remaining_ranked_batch1_application_report.json"
...
def artifact_hashes(directory: Path) -> dict[str, str]:
    return {name: sha256(directory / name) for name in PRODUCTION_FILES}
...
def validate(directory: Path, expected: dict[str, int], required_ids: set[str]) -> dict[str, int]:
    ...
    feed_ids = [story_id_from_feed_item(item) for item in items]
    feed_ids = [item for item in feed_ids if item]
    guids = [(item.findtext("guid") or "").strip() for item in items]
    if len(history_ids) != len(set(history_ids)):
        raise RuntimeError("History contains duplicate story IDs.")
    if len(feed_ids) != len(set(feed_ids)) or any(value > 1 for value in Counter(guids).values()):
    ...
    if not required_ids <= set(history_ids) or not required_ids <= set(enclosure["episodes"]) or not required_ids <= set(feed_ids):
    ...
    corrected = next(item for item in history["episodes"] if str(item.get("story_id")) == "703928648")
    if corrected.get("date") != "2019-03-15":
        raise RuntimeError("The reviewed catalog date correction is missing.")
    return counts
...
def apply(repo_root: Path = REPO_ROOT, stage_dir: Path | None = None, staging_report_path: Path | None = None) -> dict:
    ...
    if report.get("counts", {}).get("promoted_candidates") != 4:
        raise RuntimeError("Staging report does not contain the reviewed four-episode cohort.")
    required_ids = set(report["candidate_story_ids"])
    if len(required_ids) != 4:
        raise RuntimeError("Exactly four unique recovered story IDs are required.")
    ...
    staged_hashes = artifact_hashes(stage_dir)
    ...
    rollback_dir = repo_root / "work/remaining-ranked-batch1-rollback" / before["indicator_history.json"][:12]
    ...
    prepared = {}
    for name in PRODUCTION_FILES:
        temp = repo_root / f".{name}.remaining-ranked-batch1.tmp"
    ...
    try:
        ...
        for name in replaced:
            restore = repo_root / f".{name}.remaining-ranked-batch1.rollback.tmp"
