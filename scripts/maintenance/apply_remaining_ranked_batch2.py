#!/usr/bin/env python3
"""Atomically apply the reviewed three-episode ranked batch 2 staging set."""
...
PRODUCTION_FILES = ("indicator_history.json", "indicator_enclosure_map.json", "theindicator_feed.xml")
...
def validate(directory: Path, expected: dict[str, int], required_ids: set[str]) -> dict[str, int]:
    ...
    counts = {"history": len(history["episodes"]), "enclosure_map": len(enclosure["episodes"]), "feed": len(items)}
    if counts != expected:
...
def apply(repo_root: Path = REPO_ROOT, stage_dir: Path | None = None, staging_report_path: Path | None = None) -> dict:
    ...
    try:
        ...
        raise
    return {
        "report_version": 1, "mode": "production_application", "production_files_modified": True,
        "candidate_story_ids": sorted(required_ids), "counts": applied_counts,
        "staged_counts": staged_counts, "production_sha256_before": before,
        "production_sha256_after": after, "reviewed_staged_sha256": staged_hashes,
        "post_write_hashes_match_staging": after == staged_hashes,
        "rollback_directory": str(rollback_dir.relative_to(repo_root)), "rollback_files_retained": True,
    }
