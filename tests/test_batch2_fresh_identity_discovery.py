#!/usr/bin/env python3
"""
Unit tests for probe_batch2_fresh_identity_discovery.py.
...
from scripts.recovery import probe_batch2_fresh_identity_discovery as probe
...
class TestSlugify(unittest.TestCase):
    def test_basic_ascii(self):
        # apostrophe becomes a hyphen then collapses: "china-s" is the actual slugify behavior
        slug = probe.slugify("China's Social Credit System")
        self.assertNotIn("'", slug)
        self.assertIn("china", slug)
        self.assertIn("social", slug)

    def test_apostrophe(self):
        # apostrophe in "Google's" should be stripped, not hyphenated
        slug = probe.slugify("Google's Mobile Monopoly")
        self.assertNotIn("'", slug)

    def test_colon(self):
        slug = probe.slugify("China's Brave New World: Something")
        self.assertNotIn(":", slug)
    ...
    def test_no_spaces(self):
        self.assertNotIn(" ", probe.slugify("China's Social Credit System"))

    def test_lowercase(self):
        slug = probe.slugify("GOOGLE MONOPOLY")
...
class TestTitleTokenOverlap(unittest.TestCase):
    def test_identical_titles(self):
        self.assertAlmostEqual(
            probe.title_token_overlap(
                "China Social Credit System", "China Social Credit System"
            ),
            1.0,
        )
    ...
    def test_disjoint_titles(self):
        self.assertEqual(
            probe.title_token_overlap("China Social Credit System", "Cheese Imports Trade"),
            0.0,
        )
...
class TestSlugSimilarity(unittest.TestCase):
    def test_identical_slugs(self):
        self.assertAlmostEqual(
            probe.slug_similarity("china-social-credit-system", "china-social-credit-system"),
            1.0,
        )

    def test_partial_slug(self):
        score = probe.slug_similarity("china-social-credit", "china-social-credit-system")
        self.assertGreater(score, 0.0)

    def test_no_overlap(self):
        self.assertEqual(probe.slug_similarity("cheese-trade", "google-monopoly"), 0.0)
...
class TestScoreUrlForTarget(unittest.TestCase):
    ...
    def test_exact_slug_variant_match(self):
        score = self._score(
            "https://www.npr.org/sections/theindicator/2018/10/09/656123456/china-social-credit-system",
            "China's Social Credit System",
            ["china-social-credit-system", "china-social-credit"],
        )
        self.assertGreaterEqual(score, probe.SLUG_MATCH_THRESHOLD)

    def test_unrelated_story_low_score(self):
        score = self._score(
            "https://www.npr.org/sections/money/2018/10/09/999999/cheese-import-tariffs",
            "China's Social Credit System",
    ...
    def test_thematic_overlap_not_title_match(self):
        # "china-economy" shares some tokens with "china-social-credit-system" but below threshold
        score = self._score(
            "https://www.npr.org/sections/money/2018/10/09/111/china-economy-trade",
            "China's Social Credit System",
            ["china-social-credit-system"],
        )
    ...
    def test_score_between_zero_and_one(self):
        score = self._score(
            "https://www.npr.org/sections/theindicator/2018/07/23/631123456/anything",
...
class TestScorePageForTarget(unittest.TestCase):
    ...
    def test_strong_match(self):
        result = self._score(
            "China's Social Credit System : The Indicator from Planet Money",
            "2018-10-09",
            "https://www.npr.org/2018/10/09/656123456/chinas-social-credit-system",
    ...
    def test_sections_money_story_page_is_trusted(self):
        result = self._score(
            "China's Social Credit System : The Indicator from Planet Money",
            "2018-10-09",
            "https://www.npr.org/sections/money/2018/10/09/656123456/chinas-social-credit-system",
            {"has_indicator_branding": True, "program_id": "510325"},
            "2018-10-09",
            "China's Social Credit System",
        )
    ...
    def test_title_match_not_indicator_program(self):
        result = self._score(
            "China's Social Credit System : Planet Money",
            "2018-10-09",
            "https://www.npr.org/2018/10/09/656123456/chinas-social-credit-system",
    ...
    def test_indicator_date_no_title(self):
        result = self._score(
            ...
            "2018-10-09",
            "https://www.npr.org/2018/10/09/656123456/something-else",
    ...
    def test_wrong_date_no_match(self):
        result = self._score(
            "China's Social Credit System : The Indicator",
            "2020-10-13",  # wrong date
            "https://www.npr.org/2020/10/13/922262686/china-credit",
            {"has_indicator_branding": True},
            "2018-10-09",
            "China's Social Credit System",
        )
    ...
    def test_adjacent_date_accepted(self):
        result = self._score(
            "China's Social Credit System : The Indicator",
            "2018-10-08",   # 1 day before — adjacent
            "https://www.npr.org/2018/10/08/655800000/chinas-social-credit-system",
            {"has_indicator_branding": True},
            "2018-10-09",
            "China's Social Credit System",
        )
    ...
    def test_transcript_url_rejected_as_trusted_identity(self):
        result = self._score(
            "China's Social Credit System : The Indicator from Planet Money",
            "2018-10-09",
            "https://www.npr.org/templates/transcript/transcript.php?storyId=656123456",
            {"has_indicator_branding": True, "program_id": "510325"},
            "2018-10-09",
            "China's Social Credit System",
        )
    ...
    def test_sections_theindicator_url_rejected_as_trusted_identity(self):
        result = self._score(
            "China's Social Credit System : The Indicator from Planet Money",
            "2018-10-09",
            "https://www.npr.org/sections/theindicator/2018/10/09/656123456/chinas-social-credit-system",
            {"has_indicator_branding": True, "program_id": "510325"},
            "2018-10-09",
            "China's Social Credit System",
        )
...
class TestKeywordOverlapRejection(unittest.TestCase):
    """
    A page about "China" or "Google" without matching The Indicator context
    must not be treated as a confirmed identity for batch2 targets.
    """

    def test_wrong_program_rejected(self):
        # NPR story about China from ATC — title tokens overlap but not Indicator
        result = probe.score_page_for_target(
            "China Scores Its Citizens: ATC Explains",
            "2018-10-09",
            "https://www.npr.org/2018/10/09/656000000/china-scores-citizens-atc",
    ...
    def test_related_sidebar_story_rejected(self):
        # 2020 Uighur genocide report — China keywords overlap but wrong episode
        result = probe.score_page_for_target(
            "China Uighur Surveillance And The Social Credit System",
            "2020-03-10",
            "https://www.npr.org/2020/03/10/887239225/china-uighur-genocide",
            {"has_indicator_branding": False, "program_id": None},
            "2018-10-09",
            "China's Social Credit System",
        )
    ...
    def test_2020_antitrust_story_rejected_for_google_episode(self):
        # 2020 antitrust article — Google overlap with "Google's Mobile Monopoly" but wrong year
        result = probe.score_page_for_target(
            "Google And The Mobile Antitrust Case: What You Need To Know",
            "2020-10-20",   # 2 years after the target
            "https://www.npr.org/2020/10/20/925895658/google-antitrust",
...
class TestConfirmedIdentityContract(unittest.TestCase):
    def test_strong_match_verdict_requires_all_three(self):
        ...
        r = probe.score_page_for_target(
            "China's Social Credit System The Indicator",
            "2018-10-09",
            "https://www.npr.org/2018/10/09/656000000/chinas-social-credit-system",
            {"has_indicator_branding": True},
            "2018-10-09",
            "China's Social Credit System",
        )
    ...
    def test_missing_indicator_context_blocks_confirmation(self):
        r = probe.score_page_for_target(
            "China's Social Credit System",
            "2018-10-09",
            "https://www.npr.org/2018/10/09/656000000/something",
    ...
    def test_low_title_score_blocks_confirmation(self):
        r = probe.score_page_for_target(
            ...
            "2018-10-09",
            "https://www.npr.org/2018/10/09/656000000/random",
            {"has_indicator_branding": True},
            "2018-10-09",
            "China's Social Credit System",
        )
...
class TestValidatedRecoveryContract(unittest.TestCase):
    def test_indicator_url_passes_pre_flight(self):
        result = probe.validate_audio_candidate(
            "https://ondemand.npr.org/anon.npr-mp3/npr/indicator/2018/10/20181009_indicator_china.mp3",
            "2018-10-09",
        )
...
class TestAudioValidationPreflight(unittest.TestCase):
    def test_ondemand_npr_passes_to_network(self):
        r = probe.validate_audio_candidate(
            "https://ondemand.npr.org/anon.npr-mp3/npr/indicator/2018/10/file.mp3",
            "2018-10-09",
        )
    ...
    def test_podtrac_npr_passes_to_network(self):
        r = probe.validate_audio_candidate(
            "https://play.podtrac.com/npr-510325/ondemand.npr.org/anon.npr-mp3/npr/indicator/2018/10/file.mp3",
            "2018-10-09",
        )
    ...
    def test_non_audio_npr_page_passes_pre_flight(self):
        ...
        r = probe.validate_audio_candidate(
            "https://www.npr.org/2018/10/09/656000000/some-story",
            "2018-10-09",
        )
        self.assertEqual(r["validation_status"], "needs_network_check")
...
class TestCdxDateWindow(unittest.TestCase):
    def test_window_from_before_to_after(self):
        from_d, to_d = probe._cdx_date_window("2018-10-09", days_before=3, days_after=7)
...
class TestOutputFilename(unittest.TestCase):
    def test_format(self):
        self.assertEqual(
            probe.output_filename("2018-10-09"),
...
class TestTargetsConfiguration(unittest.TestCase):
    ...
    def test_no_previously_known_bad_ids_in_targets(self):
        """
        The previously known wrong IDs (from prior probes) must not appear as
        trusted starting points in this probe.
        WRONG IDs: 129451895 (ATC 2010), 922262686 (LifeKit 2020),
                   137065443 (ATC 2011), 136439885 (ME 2011),
                   887239225 (2020 Uighur story — shared China ID from ranked report),
                   925895658 (2020 antitrust article — shared Google ID from ranked report)
        """
        BAD_IDS = {129451895, 922262686, 137065443, 136439885, 887239225, 925895658}
        for t in probe.TARGETS:
...
class TestRequestBudget(unittest.TestCase):
    ...
    def test_stage_a_patterns_are_date_based(self):
        ...
        self.assertTrue(any("/2018/10/09/" in pattern for pattern in patterns))
    ...
    def test_validated_audio_requires_trusted_provenance(self):
        audio_evidence = {
            ...
            "provenance": {
                ...
                "source_capture_timestamp": "20181009120000",
                "episode_qualified": False,
                "target_episode": "2018-10-09",
                "target_title": "China's Social Credit System",
        ...
        try:
            ...
            result = probe.validate_audio_evidence_live(audio_evidence, "2018-10-09")
        finally:
...
class TestStageAPatterns(unittest.TestCase):
    ...
    def setUp(self):
        self.patterns_1009 = probe.stage_a_patterns("2018-10-09")
        self.patterns_1011 = probe.stage_a_patterns("2018-10-11")
        self.patterns_0723 = probe.stage_a_patterns("2018-07-23")

    # ---- count / budget -------------------------------------------------

    def test_count_within_budget(self):
        self.assertLessEqual(len(self.patterns_1009), probe.MAX_STAGE_A_CDX_QUERIES)
    ...
    def test_exactly_two_variants_per_day(self):
        ...
        self.assertEqual(len(self.patterns_1009), min(6, probe.MAX_STAGE_A_CDX_QUERIES))

    # ---- correct sections/money/ path ----------------------------------

    def test_all_patterns_use_sections_money(self):
        for pat in self.patterns_1009:
            self.assertIn("/sections/money/", pat, f"Pattern missing /sections/money/: {pat}")

    def test_reference_date_covered(self):
        self.assertTrue(
            any("/2018/10/09/" in p for p in self.patterns_1009),
            "No pattern covers the reference date 2018-10-09",
        )

    def test_day_before_covered(self):
        self.assertTrue(
            any("/2018/10/08/" in p for p in self.patterns_1009),
            "No pattern covers day-1 (2018-10-08)",
        )

    def test_day_after_covered(self):
        self.assertTrue(
            any("/2018/10/10/" in p for p in self.patterns_1009),
            "No pattern covers day+1 (2018-10-10)",
        )
    ...
    def test_https_variant_present(self):
        self.assertTrue(
            any(p.startswith("https://") for p in self.patterns_1009),
            "No HTTPS pattern generated",
        )

    def test_http_variant_present(self):
        self.assertTrue(
            any(p.startswith("http://") and not p.startswith("https://") for p in self.patterns_1009),
            "No HTTP pattern generated",
        )

    def test_https_and_http_are_distinct(self):
        https_patterns = [p for p in self.patterns_1009 if p.startswith("https://")]
        http_patterns = [p for p in self.patterns_1009 if p.startswith("http://") and not p.startswith("https://")]
    ...
    def test_october_11_target_sections_money(self):
        for pat in self.patterns_1011:
            self.assertIn("/sections/money/", pat)

    def test_july_target_sections_money(self):
        for pat in self.patterns_0723:
            self.assertIn("/sections/money/", pat)
