...
from scripts.analysis import stage_calculator_releases as staging


class CalculatorReleaseStagingTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.stage_dir = staging.REPO_ROOT / "work/calculator-releases-staging-test"
        cls.report = staging.json.loads(staging.REPORT.read_text(encoding="utf-8"))

    def test_every_staging_check_passes(self):
        self.assertTrue(self.report["all_checks_passed"])
        self.assertTrue(all(self.report["checks"].values()))

    def test_exact_two_release_deltas(self):
    ...
    def test_production_remains_unchanged(self):
    ...
    def test_staging_refuses_to_duplicate_applied_releases(self):
