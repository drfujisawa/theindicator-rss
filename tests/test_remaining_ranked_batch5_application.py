...
from scripts.maintenance import apply_remaining_ranked_batch5 as application


class RemainingRankedBatch5ApplicationTests(unittest.TestCase):
    def test_reviewed_staging_validates_and_hashes_match(self):
        counts = application.validate(
            application.DEFAULT_STAGE_DIR,
            {"history": 1746, "enclosure_map": 1746, "feed": 2047},
            application.REQUIRED_IDS,
        )
        self.assertEqual(counts, {"history": 1746, "enclosure_map": 1746, "feed": 2047})
